...
# class PointNet(nn.Module):
#     def __init__(self, output_points=21, alignment=False):
#         super(PointNet, self).__init__()
#         self.alignment = alignment
#
#         if self.alignment:
...
#             self.feature_transform = Transformation(64)
#
#         self.conv2s = Conv1dLayer([64, 128, 1024], act='relu', norm=True, bias=True)
#
#         self.mlp = MLP([1024, 512, 256, output_points * 3], act='relu', norm=True, bias=True, dropout=0.5)
#
#     def forward(self, x):
#         # Ensure x has the correct shape
#         if len(x.shape) == 3:
#             x = x.transpose(1, 2)  # Change shape to B x C x N
#
#         if self.alignment:
#             transform = self.input_transform(x)
#             x = torch.bmm(transform, x)
...
class PointNet(nn.Module):
    ...
    def forward(self, x):
        # Ensure x has the correct shape
        if x.size(-1) == 3:
            x = x.transpose(1, 2)  # Change shape to B x C x N if input shape is B x N x C
...
class SimpleDGCNN(nn.Module):
    def __init__(self, output_points=21, k=9):
        super(SimpleDGCNN, self).__init__()
        self.k = k

        self.convs = nn.Sequential(
            DynEdgeConv2d(3, 64, k=self.k),
            DynEdgeConv2d(64, 128, k=self.k),
            DynEdgeConv2d(128, 1024, k=self.k)
        )
        self.mlp = MLP([1024, 512, 256, output_points * 3], act='relu', norm=True, bias=True, dropout=0.5)

    def forward(self, x):
        # x shape should be [B, N, C]
        if len(x.shape) == 3:
        ...
        # use MLP to project features to the desired output shape
        out = self.mlp(global_feature)
        out = out.view(-1, 21, 3)
