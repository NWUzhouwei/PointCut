...
from models.Point_MAE import Predicted
...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    # pre_optimizer = optim.Adam(pre_model.parameters(), lr=0.001, weight_decay=0.05)
    # pre_scheduler = CosineLRScheduler(pre_optimizer,
    #             t_initial=300,
    #             t_mul=1,
    #             lr_min=1e-6,
    #             decay_rate=0.1,
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            # xyz = pre_model(points)
            points = train_transforms(points)
            loss1, loss, loss_first1, loss_first2 = base_model(points)
            loss_es = loss1 + loss + loss_first1 + loss_first2
            try:
                loss_es.backward(retain_graph=True)

                # print("Using one GPU")
            except:
                loss_es = loss_es.mean()
                loss_es.backward()
            ...
            if args.distributed:
                loss_es = dist_utils.reduce_tensor(loss, args)
            ...
            if idx % 20 == 0:
                # 打印日志
                print_log(
                    '[Epoch %d/%d][Batch %d/%d] BatchTime = %.3f (s) DataTime = %.3f (s) loss = %s loss1 = %.4f Loss_first1 = %.4f Loss_first2 = %.4f lr = %.6f ' %
                    (epoch, config.max_epoch, idx + 1, n_batches, batch_time.val(), data_time.val(),
                     ['%.4f' % l for l in losses.val()], loss1.item(), loss_first1, loss_first2, optimizer.param_groups[0]['lr']), logger=logger)
